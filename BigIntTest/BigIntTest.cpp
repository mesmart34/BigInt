...
namespace BigIntTest
{
	...
	public:
		...
		{
			auto rsa = RSA(31, 37);
			auto keys = rsa.GetKeys();

			auto msg = "hello, world";

			auto encoded = rsa.Encode(msg, get<0>(keys), get<2>(keys));			
			auto initialMsg = rsa.Decode(encoded, get<1>(keys), get<2>(keys));
